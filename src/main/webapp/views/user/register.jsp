...
<head>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .right { background:var(--cream); display:flex; align-items:center; justify-content:center; padding:2.5rem; overflow-y:auto; }
        ...
        .alt-link a { color:var(--sage); font-weight:700; text-decoration:none; }
        .alt-link a:hover { color:var(--leaf); }
        /* MOBILE */
        @media(max-width:768px) {
            body { grid-template-columns:1fr; overflow:auto; }
            .left { display:none; }
            .right { padding:1.5rem; min-height:100vh; }
...
<body>
<div class="left">
    <div class="left-brand"><div class="brand-leaf">🌿</div><div class="brand-txt">Green<em>Cart</em></div></div>
    <div class="left-hero">
...
<script>
    function checkStrength(val) {
        ...
        var s=0;
        if(val.length>=6)s++;if(val.length>=10)s++;if(/[A-Z]/.test(val))s++;if(/[0-9]/.test(val))s++;if(/[^A-Za-z0-9]/.test(val))s++;
    ...
    function checkMatch() {
        var p=document.getElementById('password').value,c=document.getElementById('confpassword').value,el=document.getElementById('matchTxt');
